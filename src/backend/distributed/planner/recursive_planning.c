...
static void WrapFunctionInSubquery(RangeTblEntry *rangeTblEntry);
...
WrapFunctionsInQuery(Query *query)
{
	...
	foreach(rangeTableCell, rangeTableList)
	{
		...
		if (rangeTableEntry->rtekind == RTE_FUNCTION &&
			FunctionReturnsSingleValue(rangeTableEntry))
		{
			WrapFunctionInSubquery(rangeTableEntry);
		}
	}
}
...
/*
 * WrapFunctionInSubquery wraps a given function RangeTableEntry
 * inside a (SELECT * from function() f) subquery.
 *
 * The said RangeTableEntry is modified and now points to the new subquery.
 * */
static void
WrapFunctionInSubquery(RangeTblEntry *rangeTblEntry)
{
	...
	if (list_length(rangeTblEntry->eref->colnames) != 1)
	{
		return;
	}
